...
syncCardButtons(); // Triggers the card UI updates instantly
}
...
// Core Sync Engine (Automatic card button UI updater)
...
// Direct +/- layout click event trigger 
function updateCardItemQty(name, factor) {
let index = cart.findIndex(item => item.name === name);
if (index !== -1) {
    adjustFpQty(index, factor);
}
}
